...
configuration may be overridden by creating a `.env` file in the root directory of the Mink backend. The `.env` file
should be formatted as a standard environment variable file, with each line containing a key-value pair. For example:
