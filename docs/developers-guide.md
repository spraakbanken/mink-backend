...
When a job has been queued, its status can be checked using the `/resource/status/get` route. The response includes details
...
call `/resource/status/get` after the annotation process completes to ensure export files are synchronized.
...
The queue manager communicates with the Mink API by calling the `/queue/advance` route.
