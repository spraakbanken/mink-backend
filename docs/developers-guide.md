...
Sparv demo mode provides anonymous, short-lived corpus processing for clients that need to annotate a small piece of
text without creating a normal Mink resource or signing in through SB Auth. Demo corpora use the regular job queue and
Sparv processing infrastructure, but have deterministic IDs, a synthetic anonymous owner, restricted exports, and an
expiry time.

### Creating and Running a Demo Corpus

Send the input text in the `text` query parameter of `POST /demo/corpus/run`. An optional `config` query parameter may
contain a complete Sparv config as plain text. If it is omitted or blank, Mink loads the config specified by
`SPARV_DEMO_DEFAULT_CONFIG`.

```bash
curl -X POST --get 'https://example.org/demo/corpus/run' \
    --data-urlencode 'text=Detta är en text.'
```

The input text is stripped of leading and trailing whitespace and must not be empty or exceed `MAX_FILE_LENGTH` bytes. A
custom config is normalized, validated against the text input, and stored with the generated resource ID inserted as its
metadata ID.

The response has the same status structure as an ordinary corpus response. The generated ID is available as
`resource.id` and normally has the form `mink-demo-<hash>`. The hash is derived from the normalized input text and
config, which has two important consequences:

- Identical text and config resolve to the same demo resource instead of creating duplicates.
- A completed or active job is reused. Mink queues processing only when the Sparv status is `none`, `aborted`, or
  `error`.

Concurrent requests for the same input are serialized with a resource-specific cache lock, preventing duplicate resource
creation and queue entries.

### Demo Corpus Endpoints

The following endpoints operate on the generated demo resource:

| Method | Endpoint | Purpose |
| --- | --- | --- |
| `POST` | `/demo/corpus/run?text=...&config=...` | Create or reuse a demo corpus and queue it when required. |
| `GET` | `/demo/corpus/input/get/{resource_id}` | Return the stored input in `input_text` and the resolved config in `config`. |
| `GET` | `/demo/corpus/status/get/{resource_id}` | Return the current job and resource status. |
| `GET` | `/demo/corpus/export/get/{resource_id}` | Return the default export file contents (usually pretty XML) directly in the `output` field. |
| `GET` | `/demo/corpus/exports/list/{resource_id}` | List created export files whose paths are allowed for demo access. |
| `GET` | `/demo/corpus/exports/download/{resource_id}?file=...` | Download one allowed export file. The `file` parameter is required; ZIP download is not supported. |
| `POST` | `/demo/corpus/job/abort/{resource_id}` | Abort the demo resource's current queued or running job. |
| `DELETE` | `/demo/corpus/remove/{resource_id}` | Abort and remove one demo corpus. Requires admin mode or the Mink secret key. |
| `DELETE` | `/demo/corpus/remove-expired` | Remove expired demo corpora. Requires admin mode or the Mink secret key. |

The input file stored by the `/demo/corpus/run` endpoint is always named `SPARV_DEMO_INPUT_FILENAME` (default:
`input.txt`). Make sure that the file extension matches the `importer` specified in the default config
(`SPARV_DEMO_DEFAULT_CONFIG`), or Sparv will fail to process it.

The `/demo/corpus/export/get/` endpoint returns the export file specified by `SPARV_DEMO_DEFAULT_EXPORT_FILE`. This file
must be generated by one of the default exports defined by `SPARV_DEMO_DEFAULT_EXPORTS`.
`SPARV_DEMO_ALLOWED_EXPORT_PATHS` limits which generated files the list and download endpoints expose.

### Access and Resource Lifetime

The run, input, status, export-get, export-list, export-download, and abort endpoints do not require authentication.
Anyone who knows a demo resource ID can use them, including aborting its current job. Demo IDs should therefore be
treated as identifiers, not credentials or a security boundary. Demo input and output must not contain sensitive
information.

Reading, downloading, aborting, or submitting the same demo resource updates its `last_accessed` timestamp. A resource
is considered expired when that timestamp is older than `SPARV_DEMO_RESOURCE_LIFETIME`, which defaults to seven days.
The queue manager periodically calls `/demo/corpus/remove-expired` using `MINK_SECRET_KEY`; administrators can also call
the endpoint manually. Cleanup rechecks expiry while holding the resource lock, so a concurrent access can refresh the
resource before it is removed.

The main demo settings are:

- `DEMO_PREFIX`: Prefix for deterministic demo resource IDs; defaults to `mink-demo-`.
- `SPARV_DEMO_CORPORA_DIR`: Directory in which demo corpora are processed.
- `SPARV_DEMO_DEFAULT_CONFIG`: Default Sparv config used when the request supplies none.
- `SPARV_DEMO_INPUT_FILENAME`: Name of the input file stored for demo corpora.
- `SPARV_DEMO_DEFAULT_EXPORTS`: Sparv exports generated by demo jobs.
- `SPARV_DEMO_DEFAULT_EXPORT_FILE`: Default export file returned by the `/demo/corpus/export/get/` endpoint.
- `SPARV_DEMO_ALLOWED_EXPORTS`: Sparv export formats that may be used by demo jobs.
- `SPARV_DEMO_ALLOWED_EXPORT_PATHS`: Export paths exposed by the list and download routes.
- `SPARV_DEMO_RESOURCE_LIFETIME`: Number of seconds without access before a demo resource expires.
- `CLEANUP_DEMO_JOBS_FREQUENCY`: How often the queue manager requests cleanup of expired demo resources.
...
Except for the explicitly public [Sparv demo routes](#sparv-demo-mode), requests that interact with resources must be
authenticated. Mink supports two authentication mechanisms:
