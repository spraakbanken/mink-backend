...
def run_sparv(oc, corpora, corpus_id):
    ...
    sparv_exports = sparv_exports.split(",") or app.config.get("SPARV_DEFAULT_EXPORTS")
    ...
    corpus_contents = utils.list_contents(oc, nextcloud_corpus_dir, exclude_dirs=False)
    ...
    try:
        utils.download_dir(oc, nextcloud_corpus_dir, local_user_dir, corpus_id, file_index)
    ...
    local_export_dir = os.path.join(local_corpus_dir, export_dir)
    try:
        utils.upload_dir(oc, nextcloud_corpus_dir, local_export_dir, corpus_id, user, file_index)
    except Exception as e:
        shutil.rmtree(local_user_dir)
