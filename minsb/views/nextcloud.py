...
def upload_corpus(oc, corpora, corpus_id):
    ...
    corpus_dir = os.path.join(current_app.config.get("CORPORA_DIR"), corpus_id)
    source_dir = os.path.join(corpus_dir, current_app.config.get("SPARV_SOURCE_DIR"))
    try:
        oc.mkdir(corpus_dir)
        oc.mkdir(source_dir)
...
def upload_config(oc, corpora, corpus_id):
    ...
    corpus_dir = os.path.join(current_app.config.get("CORPORA_DIR"), corpus_id)
    try:
        oc.put_file_contents(os.path.join(corpus_dir, "config.yaml"), config_file.read())
        return utils.success_response(f"Config file successfully uploaded for '{corpus_id}'!")
    except Exception as e:
        return utils.error_response(f"Could not upload config file for '{corpus_id}'! {e}")
...
def clear_annotations(oc, corpora, corpus_id):
    ...
    annotation_dir = os.path.join(current_app.config.get("CORPORA_DIR"), corpus_id,
                                  current_app.config.get("SPARV_WORK_DIR"))
    try:
